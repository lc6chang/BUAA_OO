		...
		if(candidate.size()==0) {
			...
			for(int i=0;i<candidate.size();i++) {
				...
				if(temp.getcredit()>best_credit) {
					...
					best_distance=GetPath.getlength(map,temp.getlocation(),request.getstart(),temp.getid());
					continue;
				}
				if(temp.getcredit()==best_credit&&GetPath.getlength(map,temp.getlocation(),request.getstart(),temp.getid())<best_distance) {
					best_id=temp.getid();
					best_credit=temp.getcredit();
					best_distance=GetPath.getlength(map,temp.getlocation(),request.getstart(),temp.getid());
					continue;
			...
			if(best_id!=100) {
				...
				/////////进行车辆单独日志记录//////////////////
				Log taxilog=new Log();
				taxilog.setRequest(request);
				taxilog.setLocation(taxilist.list[best_id].getlocation());
				taxilist.list[best_id].setcachedlog(taxilog);
				/////////////////////////////////////////////

    ...
    */
	...
	static Point randomove(CarMap map,Point point) {
		/**@REQUIRES:CarMap map,Point point;
		   ...
		   @MODIFIES:None;
		   @EFFECTS:\result==处在point点的出租车按最小流量原则随机移动的下一个坐标(只针对普通出租车);
		 */
	...
	static Point NBrandomove(CarMap map,Point point) {
		/**@REQUIRES:CarMap map,Point point;
		 			 map已经加载完毕正确的地图;
		 			 point点不超出(0,0)-(79,79)范围且不能是孤立的;
		   @MODIFIES:None;
		   @EFFECTS:\result==处在point点的出租车按最小流量原则随机移动的下一个坐标(只针对可追踪出租车);
		 */
		int boot=point.x*80+point.y;
		int[] offset=new int[]{1,-1,80,-80};
		int[] candidate=new int[4];
		...
		for(int i=0;i<4;i++) {
			int newboot=boot+offset[i];
			if(newboot>=0&&newboot<6400&&(map.getmatrix(boot,newboot)==1||map.getmatrix(boot,newboot)==2)) {
				volume[i]=map.getvolume(boot,newboot);
				if(volume[i]<minvolume) {
					minvolume=volume[i];
				}
